...
The Encoder produces bitstream files in either ITU G.192 or EVS-MIME file
storage format.
...
format specified in TS26.445 Annex A.2.6. The AMRWB-mime(RFC4867) storage
...
The package also includes a solution-file for Microsoft Visual
Studio 2017 (x86). To compile the code, please open
...
-mime            : Mime output bitstream file format
                   The encoder produces TS26.445 Annex A.2.6 Mime Storage Format, (not RFC4867 Mime Format).
...
-fec_cfg_file RF   : VoIP mode: Optimal channel aware configuration computed by the JBM
                     as described in Section 6.3.1 of TS26.448. The output is
                     written into a file named RF. Each line contains the FER indicator
                     (HI|LO) and optimal FEC offset.
-no_delay_cmp      : Turn off delay compensation
-mime              : Mime bitstream file format
                     The decoder may read both TS26.445 Annex A.2.6 and RFC4867 Mime Storage
